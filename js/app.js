...
class HackNote {
  ...
  initCanvasEditor() {
    if (!this.canvasEditor) {
      this.canvasEditor = new IframeCanvasEditor(this);
    }
...
// ============================================
// IFRAME CANVAS EDITOR (backed by hackerpad)
// ============================================

class IframeCanvasEditor {
  constructor(app) {
    this.app = app;
  ...
  listenForMessages() {
    window.addEventListener('message', (e) => {
      ...
      switch (type) {
        ...
        case 'hackerpad:changed':
          if (this.app.currentPage) {
            this.app.currentPage.canvasData = e.data.objects;
            this.app.saveData();
          }
          break;
      }
  ...
  loadFromPage(page) {
    ...
    const data = this.normalizeCanvasData(page.canvasData);
    if (this.ready) {
      this.sendLoad(data);
    } else {
  ...
  normalizeCanvasData(canvasData) {
    if (!canvasData) return { version: '1.0', objects: [] };
    // Old format: flat array of objects
    if (Array.isArray(canvasData)) {
      return { version: '1.0', objects: canvasData };
    }
    // New format: { version, objects }
    return canvasData;
  }
